...
const resume = () => {
  ...
  useEffect(()=>{
    const loadResume= async ()=>{
      ...
      //files from puter storage comes in forms of blob so we need to convert them as per need i.e image , pdf 
      const resumeBlob = await fs.read(data.resumePath);  
      if(!resumeBlob) return;

      // Convert resume blob to a pdf blob by creating a new Blob with the appropriate type and fetch url from it
